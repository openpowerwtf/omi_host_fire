...
#class WB_OMI_Host(Module, AutoCSR):
class WB_OMI_Host(Module):
   ...
   def __init__(self, module, clk, rst, wishbone, pins):
      ...
      module.specials += Instance("wb_omi_host",
         i_clk = clk,
         i_rst = rst,
...
#   from litex_boards.platforms import cmod_a7
   ...
   platform.add_source("wb_omi_host.v")
   clk = ClockSignal()
   rst = ResetSignal()
   bus = wishbone.Interface()

   wb_omi_host = platform.request('wb_omi_host')
   pins = {
   }
   module = Module()
   extmem = WB_OMI_Host(module, clk, rst, bus, pins)
