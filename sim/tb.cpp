...
int main(int argc, char **argv) {
   ...
   while (!Verilated::gotFinish() && !done) {
      ...
      tx_data = 0;// m->top->phy_tx_data;
      if (tx_data != 0) {
         /*
         cout << "cyc=" << setw(8) << setfill('0') << dec << main_time;
         cout << " phy_tx_dat=" << setw(2) << hex << tx_data << endl;
         */
      }
      rx_data = 0; //m->top->phy_rx_data;
      phy_rx_valid[0] = m->ln0_rx_valid;
      phy_rx_data[0] = m->ln0_rx_data;
      if (phy_rx_valid[0] != 0) {
         cout << "cyc=" << setw(8) << setfill('0') << dec << main_time;
         cout << " phy_rx_dat=" << setw(2) << hex << phy_rx_data[0] << endl;
      }
